...
from pynpoint.util.psf import pca_psf_subtraction, pca_psf_subtraction, IPCA
...
class IterativePcaPsfSubtractionModule(ProcessingModule):
    ...
    def run(self):
        ...
        if self.m_res_mean_out_port is not None:
            ...
            self.m_res_mean_out_port.add_history_information("IterativePcaPsfSubtractionModule", history)

        if self.m_res_median_out_port is not None:
            self.m_res_median_out_port.copy_attributes_from_input_port(self.m_star_in_port)
            self.m_res_median_out_port.add_history_information("IterativePcaPsfSubtractionModule", history)

        if self.m_res_weighted_out_port is not None:
            self.m_res_weighted_out_port.copy_attributes_from_input_port(self.m_star_in_port)
            self.m_res_weighted_out_port.add_history_information("IterativePcaPsfSubtractionModule",
                                                                 history)

        if self.m_res_rot_mean_clip_out_port is not None:
            self.m_res_rot_mean_clip_out_port.copy_attributes_from_input_port(self.m_star_in_port)
            self.m_res_rot_mean_clip_out_port.add_history_information("IterativePcaPsfSubtractionModule",
                                                                      history)
